...
const Index = ({ countProducts }: any) => {
  ...
  // Filtrar desde el front (si lo necesito esta en jpa)
...
const Index = ({ countProducts = 0 }) => {ProductType[]>([]);
  const [search, setSearch] = useState("");
  const [auxSearch, setAuxSearch] = useState("");
  const [skip, setSkip] = useState(0);
  const [page, setPage] = useState(1);
  const PageRelation = 3; //quantity of products per page
  const router = useRouter();
  const { loading } = useQuery(GET_PRODUCTS, {
    variables: {
      //search config
        ...
        OR: [
          {
            title: {
              contains: search,
              mode: "insensitive", //capital case or lower case
            },
          },
          {
            description: {
        ...
        <div className="mt-6 sm:flex sm:items-center sm:justify-between ">
          <div className="text-sm text-gray-500 dark:text-gray-400">
            ...
            <span className="font-medium text-gray-700 dark:text-gray-100">
              ...
              /* actual page of number of pages  */
          /*   </span>
          </div>
          ...
          <div className="flex items-center mt-4 gap-x-4 sm:mt-0">
            ...
            <button
              onClick={() => {
                if (page === Math.ceil(countProducts / PageRelation)) return;
                setSkip(skip + PageRelation);
                setPage(page + 1);
              }}
              className="flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md sm:w-auto gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800"
