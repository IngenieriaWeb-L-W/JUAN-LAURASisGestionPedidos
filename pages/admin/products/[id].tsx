...
const Index = ({ id }: { id: string }) => {
  ...
  const [product, setProduct] = useState<ProductType>({
    ...
    id: 0,
    description: '',
    category: { name: '', description: '', id: 0 },
  ...
  const { loading } = useQuery(GET_CATEGORIES, {
    ...
    onCompleted: (data) => {
      setOptions(
        data.categories.map((category: any) => ({ value: category.id, label: category.name }))
      );
    },
  });
  const [getProduct, { loading: loadingLazyquery }] = useLazyQuery(GET_PRODUCT, {
    fetchPolicy: 'cache-and-network',
    onCompleted: (data) => {
      console.log(data);
      setProduct(data.product);
      setSelectedOption({ value: data.product.category.id, label: data.product.category.name });
    },
  });
