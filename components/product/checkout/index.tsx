      ...
      <div className="relative col-span-9 lg:col-span-4 xl:col-span-3 bg-gray-100 lg:h-auto xl:h-screen px-8 py-14 xl:px-12 xl:py-20">
        ...
        {cart.map((product: any) => (
          ...
          >
            ...
            >
              <i
                className="iconify text-gray-800"
...
const Index = () => {
  ...
  useEffect(() => {
    if (products.length > 0 && cart.products) {
      const selected = cart.products.map((item: any) => {
  ...
  useEffect(() => {
    ...
    setTotal(productSelected.reduce((acc: any, item: any) => acc + item.price * item.quantity, 0));
  }, [productSelected]);
      ...
      <div className='relative col-span-9 lg:col-span-4 xl:col-span-3 bg-gray-100 lg:h-auto xl:h-screen px-8 py-14 xl:px-12 xl:py-20'>
        ...
        {productSelected.map((product: any) => (
          <div key={product.id} className='mt-7 flex flex-1 text-gray-800 text-lg font-normal'>
            <h3>{product.title}</h3>
            <h3 className='flex-auto text-right pr-4 md:pr-5 lg:pr-4'> {product.quantity}</h3>
            <h3>${product.price}</h3>
            <button
              type='button'
              onClick={() => {
                setProductSelected((currentCart: any) =>
                  currentCart.filter((item: any) => item.id !== product.id)
                );
              }}
              className='mx-5'
